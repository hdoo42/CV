...
\begin{minipage}[t]{0.62\textwidth} % 오른쪽 컬럼
    ...
    \begin{itemize}[label=\textbullet] 
        ...
        \begin{itemize}
            \item Rust/SQLite 기반의 올인원 워크스테이션 제어 에이전트 개발 (메모리 점유율 21.2MB 최적화)
        ...
        % [수정 반영] 인프라 모니터링: 수동 점검의 한계 극복, 즉시 알림, 디스크 임계치 언급
    ...
    \vspace{0.2em}
    42 Region (Private Cloud Build)
    \begin{itemize}[label=\textbullet]
        \descstyle 
        \item OpenStack 기반 프라이빗 클라우드 구축 중 네트워크 이슈(L2 VLAN) 트러블슈팅
